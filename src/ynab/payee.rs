...
impl Client {
    ...
    pub async fn get_payees(&self, plan_id: Uuid) -> Result<Vec<Payee>, Error> {
        let params = vec![];
        let result: PayeesDataEnvelope = self
            .get(&format!("plans/{}/payees", plan_id), &params)
            .await?;
        Ok(result.data.payees)
    }
    /// get_payee returns a single payee by ID.
    pub async fn get_payee(&self, plan_id: Uuid, payee_id: Uuid) -> Result<Payee, Error> {
        let params = vec![];
        let result: PayeeDataEnvelope = self
            .get(&format!("plans/{}/payees/{}", plan_id, payee_id), &params)
            .await?;
        Ok(result.data.payee)
    }

    /// get_payee_locations returns all locations associated with a plan.
    pub async fn get_payee_locations(&self, plan_id: Uuid) -> Result<Vec<PayeeLocation>, Error> {
        let params = vec![];
        let result: PayeeLocationsDataEnvelope = self
            .get(&format!("plans/{}/payee_locations", plan_id), &params)
            .await?;
